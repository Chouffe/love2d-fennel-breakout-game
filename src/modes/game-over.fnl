...
(fn draw-title [{: fonts : level-number}]
  ...
  (love.graphics.printf (.. "Game Over! Level " (tostring level-number) " reached.") 0 (/ config.VIRTUAL_HEIGHT 3) config.VIRTUAL_WIDTH :center)
  (love.graphics.printf (.. "Press Enter to try again") 0 (+ (/ config.VIRTUAL_HEIGHT 3) 30) config.VIRTUAL_WIDTH :center))
...
(fn keypressed [key set-mode]
  (if 
    ...
    (set-mode :start {:assets state.assets
                      :quads state.quads})))
